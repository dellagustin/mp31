...
int EGifPutLine(GifFileType *GifFile, GifPixelType *Line, int LineLen)
{
    ...
    if ((int)(Private -> PixelCount -= LineLen) < 0) {
...
int EGifPutCodeNext(GifFileType *GifFile, GifByteType *CodeBlock)
{
    ...
    if (CodeBlock != NULL) {
	if ((int)fwrite(CodeBlock, 1, CodeBlock[0] + 1, Private -> File)
	    != (int)(CodeBlock[0] + 1)) {
	    _GifError = E_GIF_ERR_WRITE_FAILED;
	    return GIF_ERROR;
	}
...
static int EGifBufferedOutput(FILE *File, GifByteType *Buf, int c)
{
	...
	if (Buf[0] != 0 && (int)fwrite(Buf, 1, Buf[0]+1, File) != (int)(Buf[0] + 1))
	{
	    _GifError = E_GIF_ERR_WRITE_FAILED;
	    return GIF_ERROR;
	}
	...
	if (Buf[0] == 255) {
	    ...
	    if ((int)fwrite(Buf, 1, Buf[0] + 1, File) !=(int)( Buf[0] + 1))
	    {
		_GifError = E_GIF_ERR_WRITE_FAILED;
		return GIF_ERROR;
	    }
